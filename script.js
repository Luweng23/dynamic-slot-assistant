...
function startSession() {
  ...
  availableBets = gameBets[gameName] || [1];
...
function saveGameHistory() {
  ...
  if (isLoss) startCountdown(index);
}
...
function renderGameHistory() {
  ...
  gameHistory.forEach((g,index) => {
    ...
    if (g.result==="WIN") {
      row.style.backgroundColor="green"; row.style.color="white";
    } else if (g.result==="LOSS") {
      row.style.backgroundColor="red"; row.style.color="white";
    } else {
      row.style.backgroundColor="black"; row.style.color="yellow";
    }
  });
}